...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ---------- Reviews carousel (vanilla: keyboard, swipe, dots) ---------- */
    var reviewsCarousel = document.getElementById('reviewsCarousel');
    var reviewsViewport = document.getElementById('reviewsViewport');
    ...
    if (reviewsCarousel && reviewsViewport && reviewsTrack && reviewsDots && reviewsPrev && reviewsNext) {
      var reviewSlides = Array.prototype.slice.call(reviewsTrack.querySelectorAll('.reviews__slide'));
      var reviewsPerView = 1;
      var reviewsPage = 0;
      var reviewsTouchStartX = null;

      // Mirrors the flex-basis breakpoints in styles.css (560px / 1024px).
      function reviewsPerViewForWidth() {
        if (window.matchMedia('(min-width: 1024px)').matches) return 3;
        if (window.matchMedia('(min-width: 560px)').matches) return 2;
        return 1;
      }

      function reviewsPageCount() {
        return Math.max(1, Math.ceil(reviewSlides.length / reviewsPerView));
      }

      function buildReviewsDots() {
        reviewsDots.innerHTML = '';
        var pages = reviewsPageCount();
        for (var i = 0; i < pages; i++) {
          var dot = document.createElement('button');
          dot.type = 'button';
          dot.className = 'reviews__dot';
          dot.setAttribute('aria-label', 'Go to testimonials page ' + (i + 1) + ' of ' + pages);
          (function (index) {
            dot.addEventListener('click', function () { goToReviewsPage(index); });
          })(i);
      ...
      function updateReviewsUI() {
        var pages = reviewsPageCount();
        reviewsDots.querySelectorAll('.reviews__dot').forEach(function (dot, i) {
          var isActive = i === reviewsPage;
          dot.classList.toggle('is-active', isActive);
          dot.setAttribute('aria-current', isActive ? 'true' : 'false');
        });
        reviewsPrev.disabled = reviewsPage === 0;
        reviewsNext.disabled = reviewsPage === pages - 1;
      }

      function goToReviewsPage(index) {
        var pages = reviewsPageCount();
        reviewsPage = Math.min(Math.max(index, 0), pages - 1);
        // Clamp so the final page never scrolls past the track's end,
        // which would otherwise leave dead space after the last card.
        var maxOffset = Math.max(0, reviewsTrack.scrollWidth - reviewsViewport.clientWidth);
        var offset = Math.min(reviewsPage * reviewsViewport.clientWidth, maxOffset);
        reviewsTrack.style.transform = 'translateX(-' + offset + 'px)';
        updateReviewsUI();
      }

      function handleReviewsResize() {
        var newPerView = reviewsPerViewForWidth();
        var perViewChanged = newPerView !== reviewsPerView;
        reviewsPerView = newPerView;
        if (perViewChanged) buildReviewsDots();
        // Re-snap to the new pixel offset without animating the resize itself.
        reviewsTrack.style.transition = 'none';
        goToReviewsPage(reviewsPage);
        void reviewsTrack.offsetHeight; // force reflow before re-enabling the transition
        reviewsTrack.style.transition = '';
      }

      reviewsPrev.addEventListener('click', function () { goToReviewsPage(reviewsPage - 1); });
      reviewsNext.addEventListener('click', function () { goToReviewsPage(reviewsPage + 1); });

      reviewsCarousel.addEventListener('keydown', function (e) {
        if (e.key === 'ArrowLeft') { e.preventDefault(); goToReviewsPage(reviewsPage - 1); }
        else if (e.key === 'ArrowRight') { e.preventDefault(); goToReviewsPage(reviewsPage + 1); }
      });

      reviewsViewport.addEventListener('touchstart', function (e) {
        reviewsTouchStartX = e.changedTouches[0].clientX;
      }, { passive: true });
      reviewsViewport.addEventListener('touchend', function (e) {
        if (reviewsTouchStartX === null) return;
        var dx = e.changedTouches[0].clientX - reviewsTouchStartX;
        if (Math.abs(dx) > 48) goToReviewsPage(dx > 0 ? reviewsPage - 1 : reviewsPage + 1);
        reviewsTouchStartX = null;
      }, { passive: true });

      var reviewsResizeTimer = null;
      window.addEventListener('resize', function () {
        clearTimeout(reviewsResizeTimer);
        reviewsResizeTimer = setTimeout(handleReviewsResize, 150);
      });

      reviewsPerView = reviewsPerViewForWidth();
      buildReviewsDots();
      goToReviewsPage(0);
    }
